...
export class GraphTransformer {
  ...
  public transform(parseResults: any[]): TransformResult {
    ...
    const relationshipIdMap = new Map<string, boolean>();
    ...
    const nodes: Node[] = new Array(totalNodeCount);
    const relationships: Relationship[] = new Array(totalRelationshipCount);
   ...
   */
  private deriveAdditionalRelationshipsInChunks(relationships: Relationship[], nodes: Node[]): Relationship[] {
    ...
    const processedDependsOnIds = new Set<string>();
  ...
  private deriveAdditionalRelationships(relationships: Relationship[], nodes: Node[]): Relationship[] {
    ...
    if (relationshipsByType['CALLS']) {
      ...
      const dependsOnObj: Record<string, DependsOn> = {};
      ...
      for (const key in dependsOnObj) delete dependsOnObj[key];
