 ...
 */
export async function createSchemaConstraints(session: Session): Promise<void> {
  console.log('Creating Neo4j schema constraints and indexes...');
  
  try {
    // Create constraints
    for (const constraint of SCHEMA_METADATA.neo4jConstraints) {
      const query = `
    ...
    // Create indexes
    for (const index of SCHEMA_METADATA.neo4jIndexes) {
    ...
    if (fullTextIndexAvailable) {
      for (const ftIndex of SCHEMA_METADATA.neo4jFullTextIndexes) {
        const query = `
 ...
 */
export async function dropSchemaConstraints(session: Session): Promise<void> {
 ...
 */
export async function createCodebaseSchema(session: Session, codebaseId: string): Promise<void> {
 ...
 */
export async function dropCodebaseSchema(session: Session, codebaseId: string): Promise<void> {
 ...
 */
export async function verifySchemaConstraints(session: Session): Promise<boolean> {
  ...
  try {
    ...
    for (const requiredConstraint of SCHEMA_METADATA.neo4jConstraints) {
    ...
    for (const requiredIndex of SCHEMA_METADATA.neo4jIndexes) {
    ...
    if (fullTextIndexAvailable) {
      for (const requiredFTIndex of SCHEMA_METADATA.neo4jFullTextIndexes) {
        const found = indexes.some(index =>
