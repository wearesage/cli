...
class PackageParser {
     ...
     */
    parseDependencies(packageJson, filePath) {
        const nodes = [];
        const relationships = [];
     ...
     */
    parseAllPackageJsonFiles(existingNodes = [], existingRelationships = []) {
     ...
     */
    linkDependenciesToImports(nodes, relationships) {
        // Find all Dependency nodes
        const dependencyNodes = nodes.filter(node => node.labels &&
            node.labels.includes('Dependency'));
        // Find all Package nodes created by the TS parser
        const packageNodes = nodes.filter(node => node.labels &&
        ...
        const importRelationships = relationships.filter(rel => rel.type === 'IMPORTS_FROM_PACKAGE');
     ...
     */
    extractPackageNameFromImport(importPath) {
